...
class HttpfulTest extends \PHPUnit\Framework\TestCase
{
    ...
    const Foreup_api_JSON_HEADER =
"HTTP/1.1 200 OK
Content-Type: application/json
Connection: keep-alive
Transfer-Encoding: chunked\r\n";
    const Foreup_api_JSON_HEADER_LOWERCASE =
        "HTTP/2 200 
...
cache-control: private, must-revalidate\r\n";
    const Foreup_api_JSON_RESPONSE = '{"key":"value","object":{"key":"value"},"array":[1,2,3,4]}';
    const Foreup_api_CSV_HEADER =
"HTTP/1.1 200 OK
Content-Type: text/csv
Connection: keep-alive
Transfer-Encoding: chunked\r\n";
    const Foreup_api_CSV_RESPONSE =
"Key1,Key2
Value1,Value2
\"40.0\",\"Forty\"";
    const Foreup_api_XML_RESPONSE = '<stdClass><arrayProp><array><k1><myClass><intProp>2</intProp></myClass></k1></array></arrayProp><stringProp>a string</stringProp><boolProp>TRUE</boolProp></stdClass>';
    const Foreup_api_XML_HEADER =
"HTTP/1.1 200 OK
Content-Type: application/xml
Connection: keep-alive
Transfer-Encoding: chunked\r\n";
    const Foreup_api_VENDOR_HEADER =
"HTTP/1.1 200 OK
Content-Type: application/vnd.nategood.message+xml
Connection: keep-alive
Transfer-Encoding: chunked\r\n";
    const Foreup_api_VENDOR_TYPE = "application/vnd.nategood.message+xml";
    const Foreup_api_MULTI_HEADER =
"HTTP/1.1 200 OK
Content-Type: application/json
Connection: keep-alive
Transfer-Encoding: chunked
...
X-My-Header:Value2\r\n";
    ...
    function testJsonResponseParse()
    {
        ...
        $response = new Response(self::Foreup_api_JSON_RESPONSE, self::Foreup_api_JSON_HEADER, $req);
    ...
    {
        $req = Request::init();
        $response = new Response(self::Foreup_api_JSON_RESPONSE, self::Foreup_api_JSON_HEADER_LOWERCASE, $req);
    ...
    function testXMLResponseParse()
    {
        ...
        $response = new Response(self::Foreup_api_XML_RESPONSE, self::Foreup_api_XML_HEADER, $req);
        $sxe = $response->body;
    ...
    function testCsvResponseParse()
    {
        ...
        $response = new Response(self::Foreup_api_CSV_RESPONSE, self::Foreup_api_CSV_HEADER, $req);
    ...
    {
        $req = Request::init()->sendsAndExpects(Mime::JSON);
        // $response = new Response(Foreup_api_JSON_RESPONSE, "", $req);
        // // Check default content type of iso-8859-1
        $response = new Response(self::Foreup_api_JSON_RESPONSE, "HTTP/1.1 200 OK
Content-Type: text/plain; charset=utf-8\r\n", $req);
    ...
    {
        $req = Request::init()->sendsAndExpects(Mime::JSON);
        $response = new Response("", self::Foreup_api_JSON_HEADER, $req);
        $this->assertEquals(null, $response->body);

        $reqXml = Request::init()->sendsAndExpects(Mime::XML);
        $responseXml = new Response("", self::Foreup_api_XML_HEADER, $reqXml);
    ...
    function testToString()
    {
        $req = Request::init()->sendsAndExpects(Mime::JSON);
        $response = new Response(self::Foreup_api_JSON_RESPONSE, self::Foreup_api_JSON_HEADER, $req);
        $this->assertEquals(self::Foreup_api_JSON_RESPONSE, (string)$response);
    }

    function test_parseHeaders()
    {
        $parse_headers = Response\Headers::fromString(self::Foreup_api_JSON_HEADER);
    ...
    {
        $req = Request::init();
        $response = new Response(self::Foreup_api_JSON_RESPONSE, self::Foreup_api_MULTI_HEADER, $req);
        $parse_headers = $response->_parseHeaders(self::Foreup_api_MULTI_HEADER);
    ...
    function testParentType()
    {
        ...
        $response = new Response('<xml><name>Nathan</name></xml>', self::Foreup_api_VENDOR_HEADER, $request);

        $this->assertEquals("application/xml", $response->parent_type);
        $this->assertEquals(self::Foreup_api_VENDOR_TYPE, $response->content_type);
...
Transfer-Encoding: chunked\r\n", $request);
    ...
    function testCustomMimeRegistering()
    {
        ...
        Httpful::register(self::Foreup_api_VENDOR_TYPE, new DemoMimeHandler());

        $this->assertTrue(Httpful::hasParserRegistered(self::Foreup_api_VENDOR_TYPE));
